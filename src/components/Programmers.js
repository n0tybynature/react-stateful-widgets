...
export default function Programmers() {
  ...
  const [ featpro, setfeatpro] = useState(null)
  ...
  const getNameOfFeatured = () => {
    ...
    return prolist.find( person => person.id === featpro).name
  ...
  const style = {
    fontSize: '1.5em',
    marginTop: '0.5em',
    color: featpro ? 'gold' : 'royalblue' // 🤔 color turns to gold, when celebrating
  };
      ...
      <div id='featured' style={style}>
        {
          ...
          prolist == false ? `🎉 Let's celebrate ${getNameOfFeatured(dev.id)}! 🥳` : 'Pick an awesome programmer'
          // false
